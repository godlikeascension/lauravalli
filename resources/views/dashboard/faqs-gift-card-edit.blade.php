...
<head>
    <meta charset="utf-8" />
    <title>Modifica FAQ Gift Card</title>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h4 class="header-title mb-0">Modifica FAQ Gift Card #{{ $faq->id }}</h4>
                                    <a href="{{ route('dashboard.faqs-gift-card.index') }}" class="btn btn-sm btn-light">
                                        <i class="mdi mdi-arrow-left"></i> Torna alle FAQ
                                    </a>
                                </div>

                                <form action="{{ route('dashboard.faqs-gift-card.update', $faq->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')

                                    <div class="mb-3">
                                        <label for="domanda" class="form-label fw-semibold">Domanda</label>
                                        <input type="text" name="domanda" id="domanda" class="form-control"
                                               value="{{ old('domanda', $faq->domanda) }}" required>
                                    </div>

                                    <div class="mb-3">
                                        <label for="risposta_html" class="form-label fw-semibold">Risposta (IT)</label>
                                        <textarea name="risposta_html" id="risposta_html" rows="8"
                                                  class="form-control">{{ old('risposta_html', $faq->risposta_html) }}</textarea>
                                    </div>
                                    ...
                                    <div class="row g-3 mb-3">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="domanda_en" class="form-control" value="{{ old('domanda_en', $faq->domanda_en) }}">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label">🇪🇸 Domanda ES</label>
                                            <input type="text" name="domanda_es" class="form-control" value="{{ old('domanda_es', $faq->domanda_es) }}">
                                        </div>
                                    </div>
                                    <div class="row g-3 mb-3">
                                        <div class="col-md-6">
                                            <label class="form-label">🇬🇧 Risposta EN</label>
                                            <textarea name="risposta_html_en" rows="5" class="form-control">{{ old('risposta_html_en', $faq->risposta_html_en) }}</textarea>
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label">🇪🇸 Risposta ES</label>
                                            <textarea name="risposta_html_es" rows="5" class="form-control">{{ old('risposta_html_es', $faq->risposta_html_es) }}</textarea>
                                        </div>
                                    </div>
                                    <hr>
                                    ...
                                    <div class="mb-3" style="max-width:160px;">
                                        ...
                                        <input type="number" name="ordine" id="ordine" class="form-control"
                                               min="0" value="{{ old('ordine', $faq->ordine) }}">
                                        <small class="text-muted">Numero più basso = prima posizione</small>
                                    </div>

                                    <div class="text-end">
                                        <a href="{{ route('dashboard.faqs-gift-card.index') }}" class="btn btn-light me-2">Annulla</a>
...
<script>
    ClassicEditor
        .create(document.querySelector('#risposta_html'), {
            toolbar: ['undo', 'redo', '|', 'bold', 'italic', 'underline', 'link', '|', 'bulletedList', 'numberedList', '|', 'removeFormat']
        })
